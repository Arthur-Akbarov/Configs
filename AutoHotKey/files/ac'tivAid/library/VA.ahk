...
; http://www.autohotkey.net/~Lexikos/VA/VA.html
;
; Modified by Wolfgang Reszel (see #Tekl comments)
...
{
	 If subunit_desc = 0 ; #Tekl
		  Return VA_GetMasterVolume(channel,device_desc) ; #Tekl
...
{
	 If subunit_desc = 0 ; #Tekl
		  Return VA_SetMasterVolume(vol,channel,device_desc) ; #Tekl
	 avl := VA_GetDeviceSubunit(device_desc, subunit_desc, "{7FB7B48F-531D-44A2-BCB3-5AD5A134B3DC}")
	 if !avl
		  return
...
{
	 If subunit_desc = 0 ; #Tekl
		  Return VA_SetMasterMute(mute,device_desc) ; #Tekl
...
{
	 If subunit_desc = 0 ; #Tekl
		  Return VA_GetMasterMute(device_desc) ; #Tekl
	 amute := VA_GetDeviceSubunit(device_desc, subunit_desc, "{DF45AEEA-B74A-4B6B-AFAD-2366B6AA012E}")
	 if !amute
		  return
...
VA_FindSubunit(device, target_desc, target_iid)
{
	 ...
	 callback := RegisterCallback("VA_FindSubunitCallback","F",2,&cbinfo)
	 r := VA_EnumSubunits(device, callback, target_name, target_iid)
	 DllCall("GlobalFree", "uint", callback)
	 return r
}

VA_FindSubunitCallback(part, interface)
{
	 NumPut(index := 1 + NumGet(A_EventInfo+4), A_EventInfo+4)
	 if NumGet(A_EventInfo+0) = index
	 {
...
VA_EnumSubunits(device, callback, target_name="", target_iid="")
{
	 ...
	 COM_Release(conn_to)
	 r := VA_EnumSubunitsEx(part, data_flow, callback, target_name, target_iid)
	 COM_Release(part)
...
VA_EnumSubunitsEx(part, data_flow, callback, target_name="", target_iid="")
{
	 ...
	 if type = 1 ; Subunit
	 {
		  ...
		  if InStr(name, target_name) ; (always true if target_name="")
		  {
				if target_iid =
					 r := DllCall(callback, "uint", part, "uint", 0, "uint")
				else
					 if VA_IPart_Activate(part, 7, target_iid, interface) = 0
					 {
						  r := DllCall(callback, "uint", part, "uint", interface, "uint")
	 ...
	 Loop %count%
	 {
		  ...
		  r := VA_EnumSubunitsEx(subpart, data_flow, callback, target_name, target_iid)
		  COM_Release(subpart)
		  if r
...
VA_GetConnector(part)
{
	 ...
	 Loop, %count%
	 {
		  ...
		  ; part->GetPartType()
		  DllCall(NumGet(NumGet(subpart+0)+24), "uint",subpart, "uint*",type)
		  If type = 0
		  {
			DllCall(NumGet(NumGet(subpart+0)+12), "uint",subpart, "uint*",subpwname)
			typename := COM_Ansi4Unicode(subpwname), COM_CoTaskMemFree(subpwname)
			COM_Release(subpart)
			break
		  }
		  Else
			typename := VA_GetConnector(subpart)
...
VA_GetSpeakersIPart(Device)
{
	 defaultDevice := VA_GetDevice(Device)
